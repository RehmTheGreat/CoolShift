    ...
    <div className="space-y-6">
      {scenarios.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border p-12 text-center bg-card">
          ...
          <p className="text-base text-foreground font-semibold">No Building Profiles Available</p>
          <p className="text-xs text-muted-foreground mt-1">Please import the public dataset to manager profiles.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {/* Left panel list */}
          <div className="md:col-span-1 space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Scenarios</h3>
                ...
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl transition-all duration-200 border ${
                    selectedId === s.scenario_id
                      ? "bg-muted border-primary/50 shadow-sm text-primary"
                      : "bg-card border-border text-muted-foreground hover:text-foreground hover:bg-muted/50"
                  }`}
                >
                  <div className="flex justify-between items-center">
                    <Badge variant="outline" className={`font-mono text-[9px] ${
                      selectedId === s.scenario_id ? "border-primary/20 text-primary" : "border-border text-muted-foreground"
                    }`}>
                      {s.scenario_id}
                    </Badge>
                    <span className="text-[10px] text-muted-foreground">{s.building_type}</span>
                  </div>
                  <h4 className="font-bold text-sm text-foreground mt-2 group-hover:text-primary">{s.name}</h4>
                </button>
              ))}
            </div>
          </div>
              ...
              <div className="space-y-6">
                <Tabs defaultValue="profile" className="w-full">
                  ...
                  <TabsContent value="profile" className="mt-4">
                    <Card className="glass-card">
                      <CardHeader className="border-b border-border">
                        <div className="flex justify-between items-center">
                          <div>
                            <CardTitle className="text-lg font-bold text-foreground">Scenario Profile Configuration</CardTitle>
                            <CardDescription className="text-xs">Adjust comfort constraints and baseline parameters.</CardDescription>
                          </div>
                          <Button
                            onClick={handleSave}
                            disabled={saving}
                            className="gradient-bg text-primary-foreground font-semibold text-xs py-1.5 px-3 flex items-center gap-1.5"
                          >
                      ...
                      <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                        ...
                        <div className="space-y-4">
                          <div className="space-y-1.5">
                            <Label htmlFor="name" className="text-xs text-muted-foreground">Profile Name</Label>
                            <Input
                              id="name"
                              value={profile.name}
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label htmlFor="area" className="text-xs text-muted-foreground">Area (m²)</Label>
                              <Input
                                id="area"
                                type="number"
                            ...
                            <div className="space-y-1.5">
                              <Label htmlFor="rooms" className="text-xs text-muted-foreground">Room Count</Label>
                              <Input
                                id="rooms"
                                type="number"
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1.5">
                              <Label htmlFor="vulnerable" className="text-xs text-muted-foreground">Vulnerable Occupants</Label>
                              <div className="flex items-center gap-2 mt-2">
                                ...
                                />
                                <span className="text-xs text-foreground/80">Infants, Elderly or Sick</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label htmlFor="comfort-min" className="text-xs text-muted-foreground">Comfort Min (°C)</Label>
                              <Input
                                id="comfort-min"
                                type="number"
                                value={profile.comfort_min_c}
                                onChange={(e) => handleProfileChange("comfort_min_c", parseFloat(e.target.value))}
                                className="bg-card border-border text-primary font-semibold"
                              />
                            </div>
                            <div className="space-y-1.5">
                              <Label htmlFor="comfort-max" className="text-xs text-muted-foreground">Comfort Max (°C)</Label>
                              <Input
                                id="comfort-max"
                                type="number"
                                value={profile.comfort_max_c}
                                onChange={(e) => handleProfileChange("comfort_max_c", parseFloat(e.target.value))}
                                className="bg-card border-border text-rose-600 dark:text-rose-400 font-semibold"
                              />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label htmlFor="insulation" className="text-xs text-muted-foreground">Insulation Level</Label>
                              <select
                                id="insulation"
                                value={profile.insulation_level}
                                onChange={(e) => handleProfileChange("insulation_level", e.target.value)}
                                className="w-full bg-card border border-border rounded-md px-3 py-2 text-foreground text-sm focus:border-primary focus:outline-none"
                              >
                            ...
                            <div className="space-y-1.5">
                              <Label htmlFor="exposure" className="text-xs text-muted-foreground">Sun Exposure</Label>
                              <select
                                id="exposure"
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label htmlFor="budget" className="text-xs text-muted-foreground">Daily Budget (PKR)</Label>
                              <Input
                                id="budget"
                                type="number"
                            ...
                            <div className="space-y-1.5">
                              <Label htmlFor="demand" className="text-xs text-muted-foreground">Max Grid Demand (kW)</Label>
                              <Input
                                id="demand"
                                type="number"
                  ...
                  <TabsContent value="appliances" className="mt-4">
                    <Card className="glass-card">
                      <CardHeader className="border-b border-border">
                        <CardTitle className="text-lg font-bold text-foreground">Cooling Appliances ({appliances.length})</CardTitle>
                      ...
                      <CardContent className="pt-6">
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-xs border-collapse">
                            ...
                            <tbody>
                              {appliances.map((app) => (
                                <tr key={app.appliance_id} className="border-b border-border text-foreground hover:bg-muted/10">
                                  <td className="p-3 font-mono font-semibold text-primary">{app.appliance_id}</td>
                                  <td className="p-3"><Badge variant="secondary" className="bg-muted text-muted-foreground">{app.zone_id}</Badge></td>
                                  ...
                                  <td className="p-3"><Badge variant="outline" className="border-primary/20 text-primary bg-primary/5">{app.efficiency_label || "N/A"}</Badge></td>
                  ...
                  <TabsContent value="assets" className="mt-4">
                    <Card className="glass-card">
                      <CardHeader className="border-b border-border">
                        <CardTitle className="text-lg font-bold text-foreground">Distributed Energy Assets</CardTitle>
                        ...
                        {!asset || (asset.solar_capacity_kw === 0 && asset.battery_capacity_kwh === 0) ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                            ...
                            {asset.solar_capacity_kw > 0 && (
                              <div className="space-y-4 p-5 rounded-xl border border-border bg-card">
                                <h4 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                                  <Sparkles className="w-4 h-4 text-primary" /> Solar Photovoltaic Generation
                                </h4>
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <p className="text-xs text-muted-foreground">PV Solar Capacity</p>
                                    <p className="text-base font-semibold text-foreground">{asset.solar_capacity_kw} kWp</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-muted-foreground">Conversion Efficiency</p>
                                    <p className="text-base font-semibold text-foreground">{(asset.solar_conversion_efficiency * 100).toFixed(0)}%</p>
                                  </div>
                                </div>
                              </div>
                            )}
                            ...
                            {asset.battery_capacity_kwh > 0 && (
                              <div className="space-y-4 p-5 rounded-xl border border-border bg-card">
                                <h4 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                                  <Zap className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Battery Storage System (BESS)
                                </h4>
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <p className="text-xs text-muted-foreground">Battery Capacity</p>
                                    <p className="text-base font-semibold text-foreground">{asset.battery_capacity_kwh} kWh</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-muted-foreground">Minimum Reserve SOC</p>
                                    <p className="text-base font-semibold text-emerald-600 dark:text-emerald-400">{asset.minimum_reserve_kwh} kWh</p>
                                  </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4 border-t border-border pt-3">
                                  <div>
                                    <p className="text-xs text-muted-foreground">Max Charge / Discharge</p>
                                    <p className="text-sm font-semibold text-foreground">{asset.max_charge_kw} kW / {asset.max_discharge_kw} kW</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-muted-foreground">Charge / Discharge Eff.</p>
                                    <p className="text-sm font-semibold text-foreground">{(asset.charge_efficiency*100).toFixed(0)}% / {(asset.discharge_efficiency*100).toFixed(0)}%</p>
                                  </div>
                                </div>
                              </div>
                            )}
